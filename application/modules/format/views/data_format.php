...
$(document).ready(function(){
	...
	table = $("#mydata").dataTable({
		...
		"serverSide": true,
		"ajax": {
			"url": "<?php echo base_url()?>index.php/format/ajax_list",
			"type": "POST"
		},
		"columnDefs":[{    
			targets: [ -1 ],
...
function reload_table()
{
    ...
    $.ajax({
    	url : "<?php echo site_url('format/ajax_edit/')?>/" + id,
    	type: "GET",
    	dataType: "JSON",
    	success: function(data)
    	{

    		$('[name="id_format"]').val(data.id);
    		$('[name="pembuka"]').val(data.pembuka);
    		$('[name="penutup"]').val(data.penutup);

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.title').text('Edit Data'); // Set title to Bootstrap modal title
...
{	
	...
	.then((willDelete) => {
		if (willDelete) {
			$.ajax({
				url : "<?php echo site_url('format/ajax_delete')?>/"+id,
				type: "POST",
				dataType: "JSON",
				success: function(data)
				{
